...
const (
	...
	// Reed-Solomon config
	dataShards   = 10 // 10 data shards  数据分区
	parityShards = 2  // 2 parity shards 校验分区
...
// Stats represents some stats about the DB state 	DB的元数据
type Stats struct {
	...
	// The number of BlobsFile， BlobsFile 的数量
	BlobsFilesCount int
...
func New(opts *Opts) (*BlobsFiles, error) {
	opts.init() //初始化blobsfile的大小
	dir := opts.Directory
	// Try to create the directory
	if err := os.MkdirAll(dir, 0700); err != nil { //0700只有拥有者有读、写、执行权限
		return nil, err
	}
	var reindex bool
	...
	// Initialize the Reed-Solomon encoder 初始化RS编码
...
// Stats returns some stats about the DB. 获取DB的信息
...
func ScanBlobsFile(path string) ([]string, error) {
	...
	//test := make([]byte, headerSize)
	//blobsfile.Read(test)
	//fmt.Println(fmt.Sprintf("%x", test))
...
func (backend *BlobsFiles) Put(hash string, data []byte) (err error) {
	...
	// Check if any async error is stored 判断有没有遗留下来的错误
	if err := backend.lastError(); err != nil {
		return err
	}

	// Ensure the data is not already stored 保证数据没有被存储过
	...
	// Encode the blob 压缩数据
	blobSize, blobEncoded := backend.encodeBlob(data, flagBlob)

	var newBlobsFileNeeded bool

	// Ensure the blosfile size won't blake2b the maxBlobsFileSize  如果要存储的文件大小超过限制的大小
	...
	if newBlobsFileNeeded {
		// Archive this blobsfile, start by creating a new one 关闭这个blobsfile，创建一个新的去存储。
		backend.n++
